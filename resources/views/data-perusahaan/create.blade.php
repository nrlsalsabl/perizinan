            ...
            <form action="{{ route('data-perusahaan.store') }}" method="POST">
                ...
                <div class="form-row">
                    <div class="form-group col-md-6">
                        <label for="nib" class="font-weight-bold">
                            <i class="fas fa-id-card mr-2"></i>NIB
                        </label>
                        <input type="text" id="nib" name="nib" class="form-control" required>
                    </div>
                    <div class="form-group col-md-6">
                        <label for="npwp" class="font-weight-bold">
                ...
                <div class="form-group">
                    <label for="nama_perusahaan" class="font-weight-bold">
                        <i class="fas fa-signature mr-2"></i>Nama Perusahaan
                    </label>
                ...
                <div class="form-group mt-4">
                    <div class="d-flex justify-content-between">
                        <a href="{{ route('data-pemohon.show') }}" class="btn btn-action btn-back">
...
@section('script')
    <script>
        $(document).ready(function() {
            ...
            const get_bentukperusahaan = () => {
                $.ajax({
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    url: "{{ route('data-perusahaan.get_bentukperusahaan') }}",
                    type: 'GET',
                    success: function(json) {
                        // Clear select options
                        $('#bentuk_perusahaan').empty();
                        // Tambahkan opsi default
                        $('#bentuk_perusahaan').append(new Option("-- No Parent --", ""));
                        // Tambahkan opsi dari data
                        $.each(json.data, function(i, item) {
                            console.log('item.id: ', item.id, 'name', item
                                .bentuk_perusahaan);
                            $('#bentuk_perusahaan').append(new Option(item
                                .bentuk_perusahaan,
                                item
                                .bentuk_perusahaan));
                        });
